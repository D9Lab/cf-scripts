// src/verify.ts
var verify = async (data, _sign, ALIST_TOKEN) => {
  ...
  const right = await hmacSha256Sign(data, expire, ALIST_TOKEN);
  if (_sign !== right) {
    return "sign mismatch";
  }
  return "";
};
var hmacSha256Sign = async (data, expire, ALIST_TOKEN) => {
  const key = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(ALIST_TOKEN),
...
var my_proxy_default = {
  async fetch(request, env) {
    ...
    let ALIST_TOKEN = env.ALIST_TOKEN;
    const sign = url.searchParams.get("sign") ?? "";
    const verifyResult = await verify(path, sign, ALIST_TOKEN);
    if (verifyResult !== "") {
      ...
      resp2.headers.set("Access-Control-Allow-Origin", requestOrigin);
    }

    url.search = '';
    // 获取自定义前缀（如果未设置则默认为空）
    let prefix = (env.PATH_PREFIX || '').trim();
    if (prefix && !prefix.startsWith('/')) {
      prefix = '/' + prefix;           // 确保以 / 开头
    }
    if (prefix.endsWith('/')) {
      prefix = prefix.slice(0, -1);    // 去除尾部 /，避免双斜杠
    }
    const rawSegments = url.pathname.split('/').filter(Boolean);
    if (rawSegments.length > 0) {
      rawSegments.shift(); // 移除 alist中的挂载名（如 "B2"）
    }
    // 拼接最终路径：前缀 + 剩余部分
    let newPath = prefix + '/' + rawSegments.join('/');
    // 避免出现连续斜杠或路径为空
